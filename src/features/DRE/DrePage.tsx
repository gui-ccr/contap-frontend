...
// Componente utilitário de linha com inversão de cores inteligente
function FinancialRow({ label, value, type = "neutral", indent = false }: FinancialItemProps) {
  ...
  // Palavras ganham cor semântica, números ficam brancos
  ...
  return (
    <div className={`flex justify-between items-center hover:bg-white/[0.02] p-xs rounded-lg transition-colors ${indent ? "pl-8" : ""}`}>
      <span className={`text-body-md flex items-center gap-2 ${labelColorClass}`}>
        {type === "negative" && <span className="material-symbols-outlined text-[16px]">remove</span>}
        {type === "positive" && <span className="material-symbols-outlined text-[16px]">add</span>}
        {label}
      </span>
      <span className="font-mono text-mono-data font-medium text-on-surface">
...
export function DrePage() {
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface flex relative overflow-hidden w-full">
      ...
      <div className="flex-1 flex flex-col min-h-screen w-full relative z-10">
        ...
        {/* TopAppBar */}
        <header className="flex justify-between items-center w-full px-margin-desktop h-16 sticky top-0 z-40 bg-surface/40 border-b border-white/5 backdrop-blur-xl">
          <div className="flex items-center gap-4">
            <span className="text-headline-md text-primary font-bold tracking-tight">ContaUp</span>
          </div>
          <div className="flex items-center gap-6">
            <span className="text-body-md text-on-surface-variant/80">19 de maio de 2026</span>
            <div className="flex items-center gap-4">
              ...
              <div className="w-8 h-8 rounded-full bg-surface-variant overflow-hidden border border-white/10">
                <div className="w-full h-full bg-gradient-to-tr from-primary/40 to-tertiary-container/40 flex items-center justify-center text-label-sm font-bold text-primary">
                  CU
                </div>
              </div>
            </div>
        ...
        {/* Conteúdo Principal */}
        <div className="flex-1 p-margin-mobile md:p-margin-desktop overflow-y-auto max-w-5xl w-full mx-auto flex flex-col gap-gutter">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <div className="relative flex-1 sm:flex-none">
                <select
                  ...
                  className="w-full appearance-none bg-surface-container-high/60 border border-white/5 rounded-xl py-2.5 pl-4 pr-10 text-label-md text-on-surface focus:outline-none focus:border-primary transition-all backdrop-blur-md"
                >
                  <option>Q3 2023 (Jul - Set)</option>
                  <option>Q2 2023 (Abr - Jun)</option>
              ...
              <button className="bg-white/[0.03] hover:bg-white/[0.07] border border-white/5 text-on-surface py-2.5 px-4 rounded-xl text-label-md font-medium transition-all flex items-center gap-2">
          ...
          <div className="flex flex-col gap-md">
            ...
            {/* Bloco 3: CARD DO RESULTADO LÍQUIDO (RECONSTRUÍDO IGUAL À FOTO DO WHATSAPP) */}
            <div className="w-full relative rounded-3xl p-10 border border-white/5 shadow-2xl bg-[#222526] text-center flex flex-col items-center justify-center gap-6">
              ...
              {/* Parágrafo Centralizado e Fluido sem quebras artificiais */}
              ...
              {/* Bloco do Valor Centralizado abaixo da descrição */}
              <div className="flex flex-col items-center gap-2 mt-2">
                <span className="text-xs text-[#00E676] font-bold tracking-widest uppercase">
                  Lucro Apurado
                </span>
                
                {/* R$ 425.000,00 Imponente e Centralizado */}
                <span className="font-sans font-bold text-[44px] md:text-[56px] leading-none tracking-tight text-[#00E676]">
                  R$ 425.000,00
                </span>
                
                {/* Pílula de Margem Líquida */}
                <div className="mt-4 inline-flex items-center gap-2 bg-[#00E676]/10 text-[#00E676] px-5 py-2 rounded-xl text-sm font-semibold border border-[#00E676]/20 shadow-sm">
