    ...
    return (
        <AriaGroup
            ...
            className={({ isFocusWithin, isDisabled, isInvalid }) =>
                cx(
                    "group/input relative flex w-full flex-row place-content-center place-items-center rounded-lg bg-surface-container-low shadow-xs ring-1 ring-outline-variant/30 transition-shadow duration-100 ease-linear ring-inset text-on-surface",
        ...
        >
            ...
            {Icon && (
                <Icon className={cx("pointer-events-none absolute text-on-surface-variant", sizes[inputSize].iconLeading, context?.iconClassName, iconClassName)} />
            )}
            ...
            <AriaDateInput {...inputProps} className={cx("flex w-full", sizes[size].root, typeof inputProps.className === "string" && inputProps.className)}>
                {(segment) => (
                    <AriaDateSegment
                        segment={segment}
                        className={cx(
                            "rounded px-0.5 tabular-nums caret-transparent focus:bg-primary focus:font-medium focus:text-on-primary focus:outline-hidden",
                            // The placeholder segment.
                            segment.isPlaceholder && "text-on-surface-variant/50 uppercase",
                            // The separator "/" segment.
                            segment.type === "literal" && "text-on-surface-variant",
                        )}
                    />
                )}
            </AriaDateInput>
            ...
            {tooltip && (
                <Tooltip title={tooltip} placement="top">
                    <TooltipTrigger
                        className={cx(
                            "absolute cursor-pointer text-on-surface-variant transition duration-200 group-invalid/input:hidden hover:text-on-surface focus:text-on-surface",
            ...
            {shortcut && (
                <div
                    className={cx(
                        "pointer-events-none absolute inset-y-0.5 right-0.5 z-10 flex items-center rounded-r-[inherit] bg-gradient-to-r from-transparent to-surface-container-low to-40% pl-8",
                        sizes[inputSize].shortcut,
                    )}
                >
                    <span
                        aria-hidden="true"
                        className="pointer-events-none rounded px-1 py-px text-xs font-medium text-on-surface-variant ring-1 ring-outline-variant/30 select-none ring-inset"
                    >
